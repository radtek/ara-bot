...
public final class DateUtil {
	 ...
	 * @return
	 */
	public static String string2Date2String(String source , String sourcePartern, String Objpartern) {
		SimpleDateFormat sdf = new SimpleDateFormat(sourcePartern);
		try {
			return convertDateToLDT(sdf.parse(source)).format(DateTimeFormatter.ofPattern(Objpartern));
		} catch (Exception e) {
			log.error("ParseException invoked {}", e);
		}
		return null;
    ...
    */
    ...
    public static String isWorkTime(String worktime, Date time) {
		String work[] = worktime.split("-");//00:00-24:00
		
		Calendar sta = Calendar.getInstance(TimeZone.getTimeZone("GMT+8:00"));
		sta.set(Calendar.HOUR_OF_DAY, Integer.parseInt(work[0].substring(0, 2)));
		sta.set(Calendar.MINUTE, Integer.parseInt(work[0].substring(3, 5)));
		Calendar end = Calendar.getInstance(TimeZone.getTimeZone("GMT+8:00"));
		end.set(Calendar.HOUR_OF_DAY, Integer.parseInt(work[1].substring(0, 2)));
		end.set(Calendar.MINUTE, Integer.parseInt(work[1].substring(3, 5)));
		log.trace("sta is {}",sta.getTime());
		log.trace("end is {}",end.getTime());
		if(sta.getTime().before(time) && end.getTime().after(time)) {
			return "on";
		}
...
//		log.info("now id is {}", DateUtil.formatNow( GlobConts.DATE_PARTERN_LONG_NONO));
		...
		log.info("result is {}",isWorkTime("00:00-24:00",DateUtil.convertLDTToDate(LocalDateTime.now())));
	}
